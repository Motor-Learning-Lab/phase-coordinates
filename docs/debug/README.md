...
| `diagnose_layer2_amplitude.py` | 7 diagnostics on the current free-sigma Layer 2 model: ridge geometry (r×sigma correlation), OLS radius, fixed-sigma comparison (seed 42 — seed-specific failure, see finding 12 note), per-chain trace, residuals breakdown, log-lik comparison true vs posterior, and ridge scan. **Key result**: corr(sigma_x, r_median)=-0.993 confirms the ridge; chains stuck from draw 1 on different ridge points; true mode is 4763 nats better than posterior mean but never found by warmup. |
