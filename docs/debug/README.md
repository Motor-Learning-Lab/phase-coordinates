...
| `diagnose_layer2_amplitude.py` | Originally: 7 diagnostics on the current free-sigma Layer 2 model (ridge geometry, OLS radius, fixed-sigma comparison, per-chain trace, residuals, log-lik comparison, ridge scan — log 11). **Rewritten (log 12)**: 6 component diagnostics (A–F) on the current free-sigma model only, per `docs/claude_followup_current_model_diagnostics.md`. No model variants. Diagnostics: (A) chain quality — rhat/ESS/per-chain sigma_x and radius; (B) phase error to true synthetic phase; (C) center cyclic amplitude and sigma_x correlations; (D) z/perpendicular compensation; (E) corrected projected radius after subtracting normal×z; (F) sigma_x correlation table across per-draw summary statistics. **Key result (log 12)**: center/orbit confounding is the dominant failure — corr(sigma_x, center_rms)=-0.999, center_rms=1.134 (comparable to orbit radius 1.0), rho_x rhat=27.93. Phase error and z are secondary. Per-draw r_projected≈1.095 (self-consistent); posterior-mean r_projected=0.311 (incompatible chain average). |
...
| `11_layer2_amplitude_diagnostics.log` | `diagnose_layer2_amplitude.py` (original 7-diagnostic version) | tune=400, draws=400, chains=2, target_accept=0.9, seed=0 (free-sigma fit); seed=42 (fixed-sigma) | **40 divergences (free-sigma), 442s.** Ridge confirmed: corr(sigma_x, r_median)=-0.993, r×sigma=0.406 (15×true). Both chains stuck from draw 1 at different ridge points (Chain 0: sigma=0.52/r=0.57; Chain 1: sigma=0.21/r=1.59). True mode log-lik=+4258 vs posterior mean=-505 (gap 4763 nats). MLE sigma for posterior trajectory=0.32 (trajectory direction itself wrong). Fixed-sigma fit (seed=42): 458 div, cos_sim 0.21 — seed-specific failure, not structural (seed=0 works per finding 9). |
| `12_layer2_current_model_diagnostics.log` | `diagnose_layer2_amplitude.py` (rewritten A–F version) | tune=400, draws=400, chains=2, target_accept=0.9, seed=0 (free-sigma only) | **40 divergences, 436s.** Full A–F diagnostics on current model only. **rho_x rhat=27.93**, Chain 1 treedepth hits 378/400 (94.5% immobile). Center/orbit confounding confirmed as dominant failure: center_rms=1.134 (≈orbit radius), corr(sigma_x, center_rms)=-0.999. Phase error RMS=0.370 rad (secondary, corr -0.777). z_rms=0.119 (secondary, corr -0.611). Per-draw r_projected=1.095 (self-consistent); posterior-mean r_projected=0.311 (incompatible chain average). Decision criterion from review doc satisfied: focus on center/radius confounding. See PROGRESS.md finding 13. |
